...
function [FOM] = Main_so(u1, timegrid, flagP)
%{
...
C_target = 0.4936;%1 [mg\L] From  TOPS study Guilhot et. al (Blood 2008) 1000 [ng/mL]
...
%% Print Doses
if flagP=='Y'
    fileID = fopen('n_doses.txt','w');
    for i=1:tot_dosi
        fprintf(fileID,'%1.0f %5.2f \n', x_i(i,1), D_i(i,1) );
    end
end
%% Absorption function
% Dimension of D_i
...
plot(t_a(:,1),y_a(:,1));
%% Print Absorption function
if flagP=='Y'
    fileID = fopen('absorption_function.txt','w');
    for i=1:Nt
