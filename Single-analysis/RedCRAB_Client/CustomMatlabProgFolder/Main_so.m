function [FOM] = Main_so(u1, timegrid)
%{
...
C_target = 1;
C_target2 = 1;
%1 [mg\L] From  TOPS study Guilhot et. al (Blood 2008) 1000 [ng/mL]
                   % 0.570 [mg\L] Form Peng et al.

%conversion factor between efficacy and stem cells decay
keff = 0.363;  
logcost= 0.43494;
a1=0.87;
p1=0.45;
...
Int_t = @(t) D_control(t,t_gate,C_p,C_target2);
F1  = integral(Int_t, 0, t_gate, 'ArrayValued',true);
% Second part of FOM
% PK DISTANCE
%Int_t = @(t) D_control(t,t_gate,C_p,C_target);
%Int_t = @(t) abs(E_target(t, t_gate, C_target) - E_control(t, t_gate, C_p) ).^(0.0001);
%Euclidean 1D



Int_t1 = @(t) abs(C_target - Concentration(t, t_gate, C_p) )^(0.1);
F2  = integral(Int_t1, 0, t_gate, 'ArrayValued',true);


%NEW F2 payoff over the number of cancer stem cells 
...
Int_t3= @(t) logcost*((2*a1-1)*p1 - keff*E_control(t, t_gate, C_p));

F4  = integral(Int_t3, 0, t_gate, 'ArrayValued',true);
